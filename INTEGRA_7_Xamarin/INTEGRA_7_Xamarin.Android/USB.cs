...
namespace INTEGRA_7_Xamarin.Droid
{
    ...
    {
        HBTrace t = new HBTrace("public class USB : BroadcastReceiver");
        ...
        {
            t.Trace("public USB(UsbManager Manager, MainActivity MainActivity)");
        ...
        {
            t.Trace("public override void OnReceive(Context context, Intent intent)");
            ...
            if (!IsReady && ACTION_USB_PERMISSION.Equals(action))
            {
                lock (this)
                {
                    ...
                    //int count = DeviceConnection.BulkTransfer(InputEndpoint, inputBuffer, 64, 5000);
                    //if (count > 0)
                    //{
                    //}
        ...
        {
            t.Trace("public void ParseForUsbDevices()");
            Int32 deviceIndex = 0;
            Int32 deviceCount = 0;
